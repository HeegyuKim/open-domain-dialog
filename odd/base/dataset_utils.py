...
def tokenize_truncate_pad(
    ...
    ids = tokenizer(texts, padding=False, truncation=False, add_special_tokens=False)["input_ids"]
    ...
    for item in ids:
        ...
        item = pad(item, pad_size, padding_side=padding_side, padding_value=tokenizer.pad_token_id)
        new_ids.append(item)

    ids = torch.tensor(new_ids, dtype=torch.long, device=device)
    inputs = {
        "input_ids": ids,
        "attention_mask": (ids != tokenizer.pad_token_id).int()
    }
